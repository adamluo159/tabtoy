...
const tsTemplate = "// Generated by github.com/adamluo159/tabtoy\n" +
	...
	"    private partLoaded = new Set<string>();\n" +
	"\n" +
	...
	"        } catch (e) {\n" +
	"            throw new Error(\"Failed to parse table data: \" + (e as Error).message);\n" +
	"        }\n" +
	"    }\n" +
	"\n" +
	"    /** 从字符串加载分片数据 */\n" +
	"    loadPartFromString(data: string, partName: string): void {\n" +
	"        try {\n" +
	"            const part = JSON.parse(data) as {{$.Name}}TablePart;\n" +
	"            \n" +
	"            switch (partName) {\n" +
	"                {{range $a, $strus := .IndexedStructs}}case \"{{$strus.Name}}\":\n" +
	"                    if (part.{{$strus.Name}}) {\n" +
	"                        this.data.{{$strus.Name}}.push(...part.{{$strus.Name}});\n" +
	"                        this.build{{$strus.Name}}Indexes();\n" +
	"                        this.partLoaded.add(partName);\n" +
	"                    }\n" +
	"                    break;\n" +
	"                {{end}}default:\n" +
	...
	"            }\n" +
	"        } catch (e) {\n" +
	"            throw new Error(\"Failed to load part \" + partName + \": \" + (e as Error).message);\n" +
	"        }\n" +
	"    }\n" +
	"\n" +
	"    /** 构建所有索引 */\n" +
