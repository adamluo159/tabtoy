...
| **SimpleInput** | 简化输入，用于结构体   | `SimpleInput:true` |
...
使用简化格式输入结构体数据，减少输入字符数。支持任意数量的字段。
...
```
ObjectType    FieldName   FieldType   Value   Meta
ItemInfo      ID          int32               SimpleInput:true
ItemInfo      Num         int32
ItemInfo      Desc        string
```

**简化格式**：

```
金币:2000:金币道具|钻石:100:钻石道具
```

**等价于标准格式**：

```
ID:金币 Num:2000 Desc:金币道具|ID:钻石 Num:100 Desc:钻石道具
```
...
1. `SimpleInput:true` 标签必须添加在**第一个字段**上
2. 简化格式为 `值1:值2:值3:...`，按字段定义顺序对应
3. 值的数量必须与字段数量一致
4. 简化格式不能包含空格（包含空格会被认为是标准格式）
5. 如果简化格式解析失败，会自动尝试标准格式
...
## 多列展开 (StructName.FieldName)

### 用途

对于 `repeated StructType` 类型的字段，可以使用多列展开的方式输入，每个单元格只填一个值，避免在单元格内写复杂的格式字符串。

### 使用方法

字段名格式：`StructName.FieldName`

系统会自动识别相同 `StructName` 的列，按顺序组成多个结构体实例。

### 示例

**@Types 表定义：**

| ObjectType | FieldName | FieldType |
| ---------- | --------- | --------- |
| ItemCost   | ID        | int32     |
| ItemCost   | Num       | int32     |
...
| Costs.ID | Costs.Num | Costs.ID | Costs.Num | Costs.ID | Costs.Num |
| -------- | --------- | -------- | --------- | -------- | --------- |
| 10000    | 2000      | 10300    | 40        | 10301    | 50        |

**等价于标准格式：**

| 字段名 | 类型                | Meta             | 值                                                    |
| ------ | ------------------- | ---------------- | ----------------------------------------------------- |
| Costs  | `repeated ItemCost` | `ListSpliter:\|` | `ID:10000 Num:2000\|ID:10300 Num:40\|ID:10301 Num:50` |
...
1. 字段名格式必须为 `StructName.FieldName`，使用点号分隔
2. `StructName` 必须是已定义的结构体类型
3. `FieldName` 必须是该结构体的字段名
4. 相同 `StructName` 的列会按出现顺序组成结构体实例
5. 多列展开格式与标准格式（`ListSpliter`）可以同时使用，互不影响

### 新旧格式对比

| 格式            | 优点                           | 缺点                     |
| --------------- | ------------------------------ | ------------------------ |
| **多列展开**    | 每个单元格只填一个值，清晰易读 | 列数较多时横向占用空间   |
| **标准格式**    | 列数少，紧凑                   | 单元格内容复杂，容易出错 |
| **SimpleInput** | 输入字符少，支持任意字段数     | 值中不能包含空格或冒号   |

根据实际场景选择合适的格式，三种格式可以混合使用。
